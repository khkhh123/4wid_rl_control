...
class CarMaker4WIDEnv(gym.Env):
    def __init__(self, loop):
        ...
        configured_protocol = os.getenv("CM_PROTOCOL_MODE", PROTOCOL_ASSESSMENT).strip().lower()
        if configured_protocol != PROTOCOL_ASSESSMENT:
            print(
                f"[WARN] CM_PROTOCOL_MODE='{configured_protocol}' is ignored. "
                f"Protocol is fixed to '{PROTOCOL_ASSESSMENT}'."
            )
        self.protocol_mode = PROTOCOL_ASSESSMENT
...
async def main():
    ...
    proj_path = Path("/home/khkhh/CM_Projects/assessment_of_4wid_ver14")
